...
def generate_result_table(sql_query:str, npar10:bool = False):
    if npar10:
    ...
    df.columns = (df.columns.to_series()
                  .apply(lambda r:
                         '\\begin{rotate}{90}' + re.sub(r'_sigma=.+', '', r.replace('e_', '')).replace('_', '\_') + '\\end{rotate}'))
...
def generate_result_table_with_ranks(sql_query:str):
    ...
    df.columns = (df.columns.to_series()
                  .apply(lambda r:
                         '\\begin{rotate}{90}' + re.sub(r'_sigma=.+', '', r.replace('e_', '')).replace("_", " ").title() + '\\end{rotate}'))
    df = df.reindex(sorted(df.columns), axis=1)
