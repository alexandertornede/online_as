...
print("FULL:")
generate_result_table("SELECT scenario_name, approach, metric, AVG(result) as avg_result, COUNT(result) FROM `server_results_ensemble_machine` WHERE metric='par10' GROUP BY scenario_name, approach, metric ORDER BY scenario_name, avg_result")
#print("PARTIAL:")
#generate_result_table("SELECT * FROM (SELECT scenario_name, approach, metric, AVG(result) as avg_result, COUNT(result) FROM `bias_linucb` WHERE metric='par10' GROUP BY scenario_name, approach, metric UNION SELECT scenario_name, approach, metric, AVG(result) as avg_result, COUNT(result) FROM `bugfix_in_bound_adaptive_lambda` WHERE metric='par10' GROUP BY scenario_name, approach, metric UNION SELECT * FROM `server_results_standard_v2_aggregated` WHERE approach = 'feature_free_epsilon_greedy' OR approach = 'degroote_EpsilonGreedy_RandomForestRegressor') as T WHERE approach != 'super_set_online_linear_regression_UCB' AND approach NOT LIKE 'degroote%%' ORDER BY scenario_name, avg_result")